...
markup = types.InlineKeyboardMarkup()
markup.add(types.InlineKeyboardButton(f"✅ إكمال التحميل ({size:.1f}MB)", callback_data=f"resume_{user_id}{file_id}"))
markup.add(types.InlineKeyboardButton("❌ حذف والبدء من جديد", callback_data=f"restart{user_id}_{file_id}"))
...
btns = [
types.InlineKeyboardButton("1080p", callback_data=f"get_{user_id}{file_id}1080"),
types.InlineKeyboardButton("720p", callback_data=f"get{user_id}{file_id}720"),
types.InlineKeyboardButton("480p", callback_data=f"get{user_id}{file_id}480"),
types.InlineKeyboardButton("360p", callback_data=f"get{user_id}{file_id}360"),
types.InlineKeyboardButton("144p", callback_data=f"get{user_id}{file_id}144"),
types.InlineKeyboardButton("🎵 MP3", callback_data=f"get{user_id}{file_id}audio"),
types.InlineKeyboardButton("⌨️ دقة يدوية", callback_data=f"manual{user_id}_{file_id}")
]
markup.add(*btns)
...
def run_task(prog_msg, user_id, url, quality, file_path):
    ...
    if success is True:
        try:
            ...
            # تنظيف المساحة (يجب أن تكون داخل try لضمان المسح بعد الرفع)
